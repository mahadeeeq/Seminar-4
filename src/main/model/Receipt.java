...
public class Receipt {
    ...
    public String toString() {
        ...
        receiptText.append("Sale Time: ").append(sale.getDateOfSale()).append(" ").append(sale.getTimeOfSale())
                .append("\n");

        List<SaleItem> items = sale.getItemList();
        for (SaleItem item : items) {
            receiptText.append(String.format("%s x%d @ %s each", item.getItem().getName(), item.getQuantity(),
                    item.getItem().getPrice().plus(item.getItem().getVat()))).append("\n");
        }
